package com.zxm.xlibray.view.popup;
...
/**
 * Created by zxm on 2016/1/14. 抽象通用popupwindow的父类
 */
public abstract class BasePopupWindow implements BasePopup {
	...
	private void initView(Activity context, int w, int h) {
		...
		navigationHeight = mContext.getResources().getDimensionPixelSize(resourceId);
//		Toast.makeText(context, "navi----->"+hasNavBar(context)+navigationHeight, Toast.LENGTH_LONG).show();
		// 默认占满全屏
	...
	public void dismissWithOutAnima() {
		try {
			if (curExitAnima != null && mAnimaView != null)
				mAnimaView.clearAnimation();
			if (curExitAnimator != null)
				curExitAnimator.removeAllListeners();
			mPopupWindow.dismiss();
		} catch (Exception e) {
			Log.d(TAG, "dismiss error");
		}
	 ...
	 * 生成ScaleAnimation
	 */
	protected Animation getScaleAnimation(float fromX, float toX, float fromY, float toY, int pivotXType,
