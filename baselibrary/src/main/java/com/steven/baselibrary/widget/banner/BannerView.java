...
import com.steven.baselibrary.utils.image.ImageLoader;
...
public class BannerView extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    private int pagerMargin = BannerConfig.PAGER_MARGIN;
    ...
    private void setImageArray(Object[] imagesUrl, OnLoadImageListener imageListener) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (imageListener != null) {
                ...
                if (defaultImage == -1)
                    ImageLoader.showImage(context,iv, url);
                else
                    ImageLoader.showImage(context,iv, url, defaultImage);
            }
        }
        setData();
    }
    ...
    public void setBannerScaleType(ScaleType scaleType) {
    ...
    private void setImageList(OnLoadImageListener imageListener) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (imageListener != null) {
                ...
                if (defaultImage == -1)
                    ImageLoader.showRoundCornerImg(context,iv, url, radius, R.mipmap.empty_big_bg);
                else
                    ImageLoader.showRoundCornerImg(context,iv, url, radius, defaultImage);
            }
        }
        setData();
    }
                ...
                public void onClick(View v) {
                    if (listener != null) {
                        ...
                        if (position == 0) {//已知当图片有8张的时候一来就向左滑动，会闪退问题，原因还不明，暂时加上这个判断
                            ...
                            if (position > size) {
                                listener.OnBannerClick(bannerBeen.get(size - 1), size - 1);
                                return;
                            }
